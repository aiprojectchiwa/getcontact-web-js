...
exports.getNumberTag = exports.authenticateWithToken = exports.setChromiumPath = void 0;
...
let chromiumPath;
const setChromiumPath = (path) => {
    chromiumPath = path;
};
exports.setChromiumPath = setChromiumPath;
            ...
            page.on('request', async (request) => {
                if (request.url().includes('https://web.getcontact.com/search')) {
                    ...
                    if (postData) {
                        ...
                        mkdir(session_dir);
                        fs_1.default.writeFileSync(`${session_dir}/session.json`, JSON.stringify(sessionData, null, 2));
 ...
 */
const getNumberTag = async (phoneNumber, countryCode = '') => {
    if (!/^\d+$/.test(phoneNumber)) {
        return { name: '', phoneNumber, provider: '', messages: ['Nomor telepon hanya boleh berisi angka.'] };
    }
    let token;
    try {
        const session = JSON.parse(fs_1.default.readFileSync('./gtc_session/session.json', 'utf-8'));
    ...
    try {
        ...
        if (numberInvalid) {
            ...
            if (numberInvalidMessage)
                messages.push(numberInvalidMessage);
        }
        ...
        if (invisibleOwner) {
            ...
            if (invisibleMessage)
                messages.push(invisibleMessage);
        }
        ...
        if (rpbiInfo) {
            result = await page.evaluate(() => {
                ...
                if (infoDiv) {
                    const name = infoDiv.querySelector('h1')?.innerText || '';
                    const phoneNumber = infoDiv.querySelector('strong > a')?.innerText || '';
                    const provider = infoDiv.querySelector('em')?.innerText.trim() || '';
...
exports.getNumberTag = getNumberTag;
async function getAllTags(phoneNumber) {
    let token;
    let hash;
    try {
        const session = JSON.parse(fs_1.default.readFileSync('./gtc_session/session.json', 'utf-8'));
